...
int train_glove() {
    ...
    if(verbose != 0) fprintf(stderr,"Initializing parameters...");
...
int Glove(char** trace, char** vectorSize, char** iter_num, char** threads, char** alpha_weight, char** cutoff, char** learn_rate, char** binary_output, char** model_output,
          
          char** vocab_input, char** output_vectors, char** save_squared_grads_file, char** input_cooccurences) {
    ...
    if (atoi(*vectorSize) > 0) vector_size = atoi(*vectorSize);
    if (atoi(*iter_num) > 0) num_iter = atoi(*iter_num);
    if (atoi(*threads) > 0) num_threads = atoi(*threads);
    cost = malloc(sizeof(real) * num_threads);
    if (atof(*alpha_weight) > 0) alpha = atof(*alpha_weight);
    if (atof(*cutoff) > 0) x_max = atof(*cutoff);
    if (atof(*learn_rate) > 0) eta = atof(*learn_rate);
    if (atoi(*binary_output) > -1) use_binary = atoi(*binary_output);
    if (atoi(*model_output) > 0) model = atoi(*model_output);
    if(model != 0 && model != 1) model = 2;
    if (*vocab_input[0]!='0') strcpy(vocab_file, *vocab_input);
    else strcpy(vocab_file, (char *)"vocab.txt");
    if (*output_vectors[0]!='0') strcpy(save_W_file, *output_vectors);
    else strcpy(save_W_file, (char *)"vectors");
    if (*save_squared_grads_file[0]!='0') {
      strcpy(save_gradsq_file, *save_squared_grads_file);
      save_gradsq = 1;
    }
    //else if(save_gradsq > 0) strcpy(save_gradsq_file, (char *)"gradsq");
    if (*input_cooccurences[0]!='0') strcpy(input_file, *input_cooccurences);
