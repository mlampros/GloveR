...
#define MAX_STRING_LENGTH1 1000
...
char train_file[MAX_STRING_LENGTH1], output_file[MAX_STRING_LENGTH1];
...
void hashinsert_COOCCUR(HASHREC **ht, char *w, long long id) {
    HASHREC	*htmp, *hprv;
    unsigned int hval = HASHFN(w, TSIZE, SEED);
    for(hprv = NULL, htmp = ht[hval]; htmp != NULL && scmp(htmp->word, w) != 0; hprv = htmp, htmp = htmp->next);
    if(htmp == NULL) {
...
int merge_files(int num) {
    ...
    if(verbose != 0) fprintf(stderr, "Merging cooccurrence files: processed 0 lines.");
    ...
    while(size > 0) {
        ...
        if((counter%100000) == 0) if(verbose != 0) fprintf(stderr,"\033[39G%lld lines.",counter);
        i = pq[0].id;
        delete(pq, size);
        fread(&new, sizeof(CREC), 1, fid[i]);
        if(feof(fid[i])) size--;
    ...
    fwrite(&old, sizeof(CREC), 1, fout);
    if(verbose != 0) fprintf(stderr,"\033[0GMerging cooccurrence files: processed %lld lines.\n",++counter);
    for(i=0;i<num;i++) {
        sprintf(filename,"%s_%04d.bin",file_head,i);
        remove(filename);
    }
...
int get_cooccurrence() {
    ...
    FILE *fid, *foverflow;
    real *bigram_table, r;
    HASHREC *htmp, **vocab_hash = inithashtable_COOCCUR();
    ...
    if(verbose != 0) fprintf(stderr, "max product: %lld\n", max_product);
    if(verbose != 0) fprintf(stderr, "overflow length: %lld\n", overflow_length);
    sprintf(format,"%%%ds %%lld", MAX_STRING_LENGTH); // Format to read from vocab file, which has (irrelevant) frequency data
    if(verbose != 0) fprintf(stderr, "Reading vocab from file \"%s\"...", vocab_file);
    fid = fopen(vocab_file,"r");
    if(fid == NULL) {fprintf(stderr,"Unable to open vocab file %s.\n",vocab_file); return 1;}
    while(fscanf(fid, format, str, &id) != EOF) hashinsert_COOCCUR(vocab_hash, str, ++j); // Here id is not used: inserting vocab words into hash table with their frequency rank, j
    fclose(fid);
    vocab_size = j;
    j = 0;
    if(verbose != 0) fprintf(stderr, "loaded %lld words.\nBuilding lookup table...", vocab_size);
    ...
    if(verbose != 0) fprintf(stderr, "table contains %lld elements.\n",lookup[a-1]);
    ...
    if(verbose != 0) fprintf(stderr,"Processing token: 0");
    ...
    while (1) {
        ...
        counter++;
        if((counter%100000) == 0) if(verbose != 0) fprintf(stderr,"\033[19G%lld",counter);
    ...
    if(verbose != 0) fprintf(stderr, "Writing cooccurrences to disk");
    fid = fopen(filename,"w");
    j = 1e6;
    for(x = 1; x <= vocab_size; x++) {
        if( (long long) (0.75*log(vocab_size / x)) < j) {j = (long long) (0.75*log(vocab_size / x)); if(verbose != 0) fprintf(stderr,".");} // log's to make it look (sort of) pretty
    ...
    if(verbose != 0) fprintf(stderr,"%d files in total.\n",fidcounter + 1);
    fclose(fid);
    fclose(foverflow);
    free(cr);
    free(lookup);
...
int cooccurrence_statistics(char** train_data, char** vocab_input, char** output_cooccurences, char** symmetric_both, char** context_words, char** memory_gb, char** MAX_product, char** overflowLength, char** trace) {
    int i;
    real rlimit, n = 1e5;
    ...
    if (atoi(*symmetric_both) > -1) symmetric = atoi(*symmetric_both);
    if (atoi(*context_words) > -1) window_size = atoi(*context_words);
    if (*vocab_input[0]!='0') strcpy(vocab_file, *vocab_input);
    ...
    if (atof(*memory_gb) > 0) memory_limit = atof(*memory_gb);
    ...
    if (atoll(*MAX_product) > 0) max_product = atoll(*MAX_product);
    if (atoll(*overflowLength) > 0) overflow_length = atoll(*overflowLength);
